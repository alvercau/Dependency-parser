...
class FeatureExtractor(object):
    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        buffer_idx0 = buffer_idx1 = buffer_idx2 = False
        if len(buffer) >= 3:
...
# add a feature that indicates the distance between the top of the stack and the first word in the input buffer.
# the distance is the number of words intervening.

        if stack_idx0 and buffer_idx0:
            token_stack = tokens[stack_idx0]
            token_buffer = tokens[buffer_idx0]
            distance = token_buffer['address']-token_stack['address']
            result.append('DISTANCE_0_'+str(distance))
